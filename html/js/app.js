    ...
    $.post(
        ...
        function(data) {
            ...
            $(".weapon-attachments-container-details").html(
                '<span style="font-weight: bold; letter-spacing: .1vh;">Serie</span><br> ' +
                ClickedItemData.info.serie +
                '<br><br><span style="font-weight: bold; letter-spacing: .1vh;">Varighet - ' +
                Durability.toFixed() +
            ...
            if (data.AttachmentData !== null && data.AttachmentData !== undefined) {
                if (data.AttachmentData.length > 0) {
                    ...
                    $(".weapon-attachments-title").html(
                        '<span style="font-weight: bold; letter-spacing: .1vh;">Denne pistolen inneholder ikke vedlegg</span>'
                    );
                }
            } else {
                $(".weapon-attachments-title").html(
...
$(document).on("click", "#weapon-attachments", function(e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post(
            ...
            JSON.stringify({
                message: "Vedlegg er utilgjengelige for denne pistolen.",
                type: "error",
            })
        );
    }
    ...
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            $(".item-info-description").html(
                ...
                "</span></p><p><strong>Etternavn: </strong><span>" +
                itemData.info.lastname +
                "</span></p><p><strong>Fødselsdato: </strong><span>" +
                itemData.info.birthdate +
                "</span></p><p><strong>Kjønn: </strong><span>" +
                gender +
                "</span></p><p><strong>Nasjonalitet: </strong><span>" +
            ...
            $(".item-info-description").html(
                ...
                "</span></p><p><strong>Fødselsdato: </strong><span>" +
                itemData.info.birthdate +
                "</span></p><p><strong>Lisenser: </strong><span>" +
                itemData.info.type +
                "</span></p>"
            );
            ...
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
                "<p>" + "Et sett som brukes til å suge bensin fra kjøretøy! <br><br>" + itemData.info.gasamount + " Liter igjen.</p>" +
                "</span></p><p style=\"padding-top: .8vh;font-size:11px\"><b>Vekt: </b>" + ((itemData.weight * itemData.amount) / 1000).toFixed(1) + " | <b>Antall: </b> " + itemData.amount
            );
        } else if (itemData.name == "jerrycan") { // Jerry Can (CDN-Fuel!)
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
                "<p>" + "En Bensinkanne, designet for å holde drivstoff! <br><br>" + itemData.info.gasamount + " Liter igjen.</p>" +
                "</span></p><p style=\"padding-top: .8vh;font-size:11px\"><b>Vekt: </b>" + ((itemData.weight * itemData.amount) / 1000).toFixed(1) + " | <b>Antall: </b> " + itemData.amount
            );
        } else if (itemData.name == "syphoningkit") { // Syphoning Kit (CDN-Fuel or CDN-Syphoning!)
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            ...
            if (itemData.info.attachments != null) {
                ...
                $(".item-info-description").html(
                    "<p><strong>Serie: </strong><span>" +
                    itemData.info.serie +
                    "</span></p><p><strong>Ammunisjon: </strong><span>" +
                    itemData.info.ammo +
                    "</span></p><p><strong>Vedlegg: </strong><span>" +
                    attachmentString +
                    "</span></p>"
                );
            } else {
            ...
            if (itemData.info.type == "casing") {
                $(".item-info-description").html(
                    ...
                    itemData.info.label +
                    "</span></p><p><strong>Skriv nummer: </strong><span>" +
                    itemData.info.ammotype +
                    "</span></p><p><strong>Kaliber: </strong><span>" +
                    itemData.info.ammolabel +
                    "</span></p><p><strong>Serie: </strong><span>" +
                    itemData.info.serie +
                    "</span></p><p><strong>Åsted: </strong><span>" +
                ...
                $(".item-info-title").html('<p>' + itemData.label + '</p>')
                $(".item-info-description").html('<p><strong>Name: </strong><span>'+ itemData.info.firstname + '</span></p><p><strong>Etternavn: </strong><span>'+ itemData.info.lastname+ '</span></p><p><strong>Skilt: </strong><span>'+ itemData.info.plate + '<p><strong>Modell: </strong><span>'+ itemData.info.model +'</span></p>');
            }
        } else if (
            ...
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
                "<p><strong>Mengde kontanter: </strong><span>kr" +
                itemData.info.cash +
                "</span></p>"
            );
            ...
            if (dragAmount == 0) {
                if (itemData.price != null) {
                    ...
                    $(".ui-draggable-dragging")
                        ...
                        .html(" " + itemData.amount + " NOK " + itemData.price);
                    ...
                    $.post(
                        ...
                        function(item) {
                            $(".combine-option-text").html(
                                "<p>Hvis du kombinerer disse varene du får: <b>" +
                                item.label +
                                "</b></p>"
                            );
                        }
...
(() => {
    ...
    Inventory.Open = function(data) {
        ...
        $(".ply-iteminfo-container").css("opacity", "0.0");
        $('.namejs').html('<i class="fa fa-id-badge"></i> '+v.pid+' | '+v.playername+' | '+'<i class="fas fa-wallet"></i>'+' $ '+v.money) //show ID, Name and Cashs
    ...
    Inventory.UseItem = function(data) {
        ...
        $("#itembox-action").html("<p>Brukte 1x</p>");
    ...
    Inventory.itemBox = function(data) {
        ...
        var type = "Brukte " + data.itemAmount + "x";
        if (data.type == "add") {
          type = "Mottok " + data.itemAmount + "x";
        } else if (data.type == "remove") {
          type = "Fjernet " + data.itemAmount + "x";
        }
        ...
        if (data.toggle) {
            if (!requiredItemOpen) {
                ...
                $.each(data.items, function(index, item) {
                    var element =
                        '<div class="requiredItem-box"><div id="requiredItem-action">Påkrevd</div><div id="requiredItem-label"><p>' +
                        item.label +
    ...
    window.onload = function(e) {
        window.addEventListener("message", function(event) {
            switch (event.data.action) {
                ...
                case "RobMoney":
                    $(".inv-options-list").append(
                        '<div class="inv-option-item" id="rob-money"><p>TA PENGER</p></div>'
                    );
