...
local function AddItem(source, item, amount, slot, info)
	...
	if not itemInfo and not Player.Offline then
		QBCore.Functions.Notify(source, "Varen eksisterer ikke", 'error')
		return false
	end
	...
	elseif not Player.Offline then
		QBCore.Functions.Notify(source, "Inventory er for full", 'error')
	end
	return false
end
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		TriggerClientEvent("QBCore:Notify", source, "Du har ikke denne varen!", "error")
		return
	end
end
...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		...
		TriggerClientEvent('QBCore:Notify', src, 'Ikke tilgjengelig', 'error')
	end
end)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				if IsItemValid then
					...
					TriggerClientEvent('QBCore:Notify', src, "Du kan ikke selge denne varen..", 'error')
				end
			else
				-- drop
			...
			TriggerClientEvent("QBCore:Notify", src, "Du har ikke denne varen!", "error")
		end
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			TriggerClientEvent("QBCore:Notify", src, "Varen eksisterer ikke??", "error")
		end
		...
		if QBCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					QBCore.Functions.Notify(src, itemInfo["label"] .. " kjøpt!", "success")
					...
					QBCore.Functions.Notify(src, "Du har ikke nok kontanter..", "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('QBCore:Notify', src, "Du har ikke de riktige varene..", "error")
		end
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
	...
	if Player == OtherPlayer then return QBCore.Functions.Notify(src, "Kan du ikke gi deg selv en gjenstand") end
	if dist > 2 then return QBCore.Functions.Notify(src, "Du er for langt unna til å gi ting!") end
	local item = GetItemBySlot(src, slot)
	if not item then QBCore.Functions.Notify(src, "Varen du prøvde å ikke finne!"); return end
	if item.name ~= name then QBCore.Functions.Notify(src, "Feil gjenstand funnet prøv igjen!"); return end
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info) then
				...
				QBCore.Functions.Notify(target, "Du mottok "..amount..' '..item.label.." Fra "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, QBCore.Shared.Items[item.name], "remove")
				QBCore.Functions.Notify(src, "Du ga " .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				...
				QBCore.Functions.Notify(src, "De andre spillerbeholdningen er full!", "error")
				QBCore.Functions.Notify(target, "De andre spillerbeholdningen er full!", "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
			end
		else
			TriggerClientEvent('QBCore:Notify', src,  "Du har ikke nok av varen ", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Du har ikke nok varer til å overføre")
	end
end)
...
QBCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType and invId then
		...
		TriggerClientEvent('QBCore:Notify', source,  "Argumenter ikke fylt ut riktig..", "error")
	end
end, "admin")

QBCore.Commands.Add("ran", "Ran en spiler", {}, false, function(source, args)
	...
	if Player then
			if itemData then
				...
				if AddItem(id, itemData["name"], amount, false, info) then
					...
					QBCore.Functions.Notify(source, "Kan ikke gi vare!", "error")
				end
			else
				QBCore.Functions.Notify(source, "Varen eksisterer ikke", "error")
			end
...
QBCore.Commands.Add('clearinv', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    ...
    if Player then
        ...
        QBCore.Functions.Notify(source, "Spiller ikke online", 'error')
    end
end, 'admin')
	...
	for _, v in pairs(players) do
		...
		if dist < 3.0 then
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Fornavn:</strong> {1} <br><strong>Etternavn:</strong> {2} <br><strong>Fødelsdato:</strong> {3} <br><strong>Lisenser:</strong> {4}</div></div>',
					args = {
						"Førerkort",
						item.info.firstname,
						item.info.lastname,
...
CreateUsableItem("id_card", function(source, item)
	...
	for _, v in pairs(players) do
		...
		if dist < 3.0 then
			...
			TriggerClientEvent('chat:addMessage', v,  {
					template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}:</strong><br><br> <strong>Borger ID:</strong> {1} <br><strong>Fornavn:</strong> {2} <br><strong>Etternavn:</strong> {3} <br><strong>Fødselsdato:</strong> {4} <br><strong>Kjønn:</strong> {5} <br><strong>Nasjonalitet:</strong> {6}</div></div>',
					args = {
						"ID Kort",
						item.info.citizenid,
						item.info.firstname,
